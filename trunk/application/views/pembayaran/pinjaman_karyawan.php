...
<div id="content" style="width:100%; height:550px;">
	...
	<table align="left" style="width:25%;margin-right:25px">
			<tr><td style="text-align:left"><span>NIK</span> <span style="float:right">:<input type="text" name="nik_karyawan" id="nik_karyawan" class="normal_textbox"></div></td></tr>
			<tr><td style="text-align:left"><span>Nama</span> <span style="float:right">:<input type="text" name="nama_karyawan" id="nama_karyawan" class="normal_textbox"></div></td></tr>
	...
	$(function(){
		...
		$("#button_save").click(function(){
			...
			if(nik==""){
				$("#nik_karyawan").css("background-color","#ff6666");
				$("#nik_karyawan").focus();
				return false;
			}
			
			if(nm==""){
				$("#nama_karyawan").css("background-color","#ff6666");
				$("#nama_karyawan").focus();
				return false;
			}
			if(idp==""){
			...
			$.ajax({
				url 	: '<?=site_url()?>pembayaran/post_pinjaman',
				type 	: 'post',
				data 	: {'nik':nik, 'nm':nm, 'sd':sd, 'idp':idp, 'bgt':bgt,},
	...
	<table style="width:675px">
		<thead>
			<tr>
				<th colspan=5>
					<span style="float:left;padding:5px"><b>Pinjaman Karyawan</b></span>
				</th>
			</tr>
		</thead>
		<thead>
		...
		<tbody id="account_budget">
			<?php $pinjaman = $this->db->query("select * from hutang_pegawai_tbl")->result();?>
			...
			<tr>
				...
				<td>
					<a style="text-decoration:none" href="<?=site_url()?>pembayaran/detail_pinjaman/<?=htmlspecialchars($p->nik_pegawai);?>" data-tooltip="Alokasi Dana"  class="default_popup">
