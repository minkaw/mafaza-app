...
<head><link rel="shortcut icon" href="<?=site_url()?>assets/img-web/pavicon_sd.png"/><title>Sekolah Dasar</title>
	...
	$(function(){
		...
		$("#laporan-view").click(function(){					
			...
			window.open("<?=site_url()?>laporan/load_laporan_iuran_siswa/"+encodeURIComponent(iu)+"/"+encodeURIComponent(sd)+"/"+encodeURIComponent(ed)+"", 'window name', 'window settings');
		});
	...
	<table align="" style="width:20%;margin-right:25px">
			<tr><td>
				...
				<span style="float:right">
					:
					<select id="iuran_choice" style="width:150px;height:23px">
						<?php $iuran = $this->db->query("SELECT id_acc_pembayaran, nama_acc_pembayaran FROM account_pembayaran_tbl")->result();?>
						<?php foreach($iuran as $i){ ?>
						<option value="<?=$i->id_acc_pembayaran."--spr--".$i->nama_acc_pembayaran;?>"><?=$i->nama_acc_pembayaran;?></option>
						<?php } ?>
					</select>
				</span>
			</td></tr>
			...
			<tr>
				<td style="text-align:right">
					<button id="laporan-view" value="budget" class="btn-view2">View</button>
				</td>
			</tr>
	</table>
	</span>
