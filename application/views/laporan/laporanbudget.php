...
<head><link rel="shortcut icon" href="<?=site_url()?>assets/img-web/pavicon_sd.png"/><title>Sekolah Dasar</title>
	...
	$(function(){
		...
		$("#laporan-view").click(function(){					
			...
			window.open("<?=site_url()?>laporan/load_laporanbudget/"+encodeURIComponent(bd)+"/"+encodeURIComponent(sd)+"/"+encodeURIComponent(ed)+"", 'window name', 'window settings');
			//$("load_laporanbudget").load("<?=site_url()?>laporan/load_laporanbudget/"+encodeURIComponent(bd)+"/"+encodeURIComponent(sd)+"/"+encodeURIComponent(ed)+"");
		});
	...
	<table align="" style="width:20%;margin-right:25px">
			<tr><td>
				...
				<span style="float:right">
					:
					<select id="budget_choice" style="width:150px;height:23px">
						<option value="all">ALL</option>
						<?php foreach($budget as $b){ ?>
						<option value="<?=$b->id_budget;?>"><?=$b->nama_budget;?></option>
						<?php } ?>
					</select>
				</span>
			</td></tr>
