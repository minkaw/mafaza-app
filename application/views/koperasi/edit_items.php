<script type="text/javascript">
	...
	$(function(){
		...
		$("#edit_item").click(function(){
			...
			$.ajax({
				url 	: '<?=site_url()?>koperasi/update_itemkoperasi',
				type 	: 'post',
				data 	: {'n':n,'s':s,'h':h,'i':i},
...
<table style="width:350px">
	...
	<tr>
		...
		<td><input type="text" id="nama_item" value="<?=$item->nama_item?>" class="normal_textbox"></td>
	</tr>
	<tr>
		<td style="text-align:left">Satuan</td>
		<td>
			<select class="class_select1" id="satuan_item">
				<option <?php if($item->satuan=="pcs"){ echo "selected"; } ?> value="pcs">PCS</option>
				<option <?php if($item->satuan=="paket"){ echo "selected"; } ?> value="paket">Paket</option>
			</select>
		</td>
	</tr>
	<tr>
		<td style="text-align:left">harga</td>
		<td><input type="text" class="normal_textbox" value="<?=number_format($item->harga)?>" id="harga_item"></td>
	</tr>
	<tr>
		<td colspan="2">
			<span style="float:right">
			<button id="edit_item" class="btn-save" >Simpan</button>
