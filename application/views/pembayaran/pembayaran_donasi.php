...
<div id="content" style="width:100%; height:530px;">
	...
	<table style="width:55%">
		<thead>
			<tr>
				<th colspan=4>
				<a style="text-decoration:none;float:right" href="<?=site_url()?>pembayaran/input_donatur" class="default_popup">
				<button class="btn-add" style="width:120px;background-color:#63a37f;text-align:right">Input Donatur</button>
				</a>
				</th>
			</tr>
			<tr>
		...
		<!-- Result  -->
		<tbody id="donatur_id">
			<?php $no=1; foreach($donasi as $i){?>
			<tr>
				<td><?=$no;?></td>
				<td><?=$i->nama_donatur;?></td>
				<td style="text-align:right"><?=number_format($i->jumlah_donatur);?></td>
				<td><?=indo_date($i->tgl_donatur);?></td>
			</tr>
			<?php $no++;} ?>
		</tbody>
	</table>
